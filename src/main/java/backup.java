...
//
//        model = new DefaultTableModel(new String[]{"STT", "Scope", "URL", "API", "Status"}, 0);
//        table = new JTable(model);
//        table.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
//
//        // Kích thước cột
//        table.getColumnModel().getColumn(0).setPreferredWidth(20);  // STT nhỏ hơn
//        table.getColumnModel().getColumn(1).setPreferredWidth(200); // Scope
//        table.getColumnModel().getColumn(2).setPreferredWidth(200); // URL
//        table.getColumnModel().getColumn(3).setPreferredWidth(250); // API
...
//
//
//        panel.add(buttonPanel, BorderLayout.NORTH);  // Di chuyển lên trên
//        panel.add(scrollPane, BorderLayout.CENTER);  // Scroll nằm dưới
//
//
...
//            if (parsedUrl.getPort() != -1) scope += ":" + parsedUrl.getPort();
//            scope += "/";
//
//            model.addRow(new Object[]{model.getRowCount() + 1, scope, fullUrl, apiExtracted, inSiteMap ? "✅" : "❌"});
//        } catch (Exception e) {
//            e.printStackTrace();
//        }
//    }
...
//                if (cols.length == 5) {
//                    Object[] cleaned = new Object[5];
//                    for (int i = 0; i < 5; i++) cleaned[i] = cols[i].replaceAll("^\"|\"$", "");
//                    seenApis.add((String) cleaned[3]);
...
//            if ("❌".equals(status)) {
//                String apiPath = model.getValueAt(row, 3).toString();
